...
const VendorDashboard = ({ initialTab = 'home' }) => {
  ...
  const isVerified = user?.vendorStatus === 'verified';
  ...
  const handleAddProduct = async (e) => {
    ...
    if (!validatePriceAndStock()) return;
    ...
    try {
      ...
      // Show message returned by the AI decision engine
      const serverMessage = response.data?.message;
      showToast(serverMessage || 'Product submitted successfully!', 'success');
  ...
  const getStatusBadge = (status) => {
    ...
    const styles = {
      ...
      'Pending Vendor Action': 'bg-amber-500 text-white',
      'In Progress': 'bg-blue-500 text-white',
      'Shipped': 'bg-purple-500 text-white',
      'Delivered': 'bg-green-500 text-white',
      'Cancelled': 'bg-gray-500 text-white'
    };
    return styles[normalized] || styles[status] || styles['Pending'];
  };
  ...
  return (
    <div className="min-h-screen bg-secondary-900">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <h1 className="font-display text-4xl font-bold">Vendor Hub</h1>
                  {isVerified && (
                    <div className="flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-green-500 to-cyan-500 rounded-full shadow-lg shadow-green-500/40 border border-green-400/50">
                      <Shield className="w-5 h-5 text-white" />
                      <span className="text-white font-semibold text-sm tracking-wide">VERIFIED</span>
                    </div>
                  )}
                </div>
      ...
      {!isVerified && (
        <div className="bg-gradient-to-r from-yellow-900/50 to-orange-900/50 border-b border-yellow-500/30">
          <div className="container-custom py-4">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-yellow-300">Account Not Verified</h3>
                  <p className="text-yellow-200/80 text-sm">
                    Complete verification to unlock product creation, order management, and AR features.
                  </p>
                </div>
              </div>
              <Link
                to="/vendor/verification"
                className="flex items-center gap-2 px-6 py-2.5 bg-yellow-500 hover:bg-yellow-400 text-black font-semibold rounded-lg transition-all duration-300 shadow-lg shadow-yellow-500/30"
              >
      ...
      <div className="bg-surface border-b border-surface-light sticky top-0 z-40">
        <div className="container-custom">
          <div className="flex gap-1">
            {[
              { id: 'home', label: 'My Products', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>, requiresVerification: false },
              { id: 'orders', label: 'Orders', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>, requiresVerification: true },
            ].map(tab => {
              const isDisabled = tab.requiresVerification && !isVerified;
              return (
                <button
                  key={tab.id}
                  onClick={() => {
                  ...
                  className={`flex items-center gap-2 px-6 py-4 font-medium transition-all duration-300 border-b-2 ${
                    isDisabled
                      ? 'text-text-tertiary cursor-not-allowed border-transparent'
                      : activeTab === tab.id
                ...
                >
                  {isDisabled ? <Lock className="w-4 h-4" /> : tab.icon}
                  <span>{tab.label}</span>
                  {isDisabled && (
                    <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full">Locked</span>
                  )}
                </button>
              );
            })}
      ...
      <div className="container-custom py-8">
          ...
          <div>
            ...
            <div className="mb-8 flex items-center justify-between">
              ...
              {isVerified ? (
                ...
                <div className="flex items-center gap-2 px-6 py-3 bg-surface-light text-text-tertiary rounded-xl font-semibold cursor-not-allowed">
                  <Lock className="w-5 h-5" />
                  <span>Add New Product</span>
                  <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full ml-2">Verification Required</span>
                </div>
              )}
            </div>
            ...
            {showAddProduct && (
              <form onSubmit={handleAddProduct} className="mb-8 bg-surface border border-surface-light rounded-xl p-6 shadow-lg">
                ...
                <button
                  ...
                  className="mt-6 px-8 py-3 bg-gradient-to-r from-primary-800 to-primary-700 hover:from-primary-700 hover:to-primary-600 text-white rounded-xl font-semibold transition-all duration-300 shadow-lg shadow-primary-700/30 disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-3"
                >
                  {isSubmittingProduct ? (
                    <>
                  ...
                  <div key={product._id} className="bg-surface border border-surface-light rounded-xl overflow-hidden hover:border-primary-500/50 transition-all duration-300 hover:shadow-xl group">
                    ...
                    <div className="relative h-48 bg-secondary-900 overflow-hidden">
                      ...
                      <div className="absolute top-3 right-3 flex flex-col items-end gap-1">
                        ...
                        {product.aiDecision === 'pending_review' && product.approvalStatus === 'Pending' && (
                          <div className="px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide bg-blue-900/70 text-blue-300 border border-blue-700/50">
                            Under Review
                          </div>
                        )}
                      </div>
                    ...
                    <div className="p-5">
                      ...
                      {product.approvalStatus === 'Approved' && (
                        <div className="mb-4 p-3 bg-green-900/20 border border-green-600/30 rounded-lg flex items-start gap-2">
                          <svg className="w-4 h-4 text-green-400 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <p className="text-xs text-green-300">Your product is now live on the platform.</p>
                        </div>
                      )}

                      {/* ── Pending (AI flagged) ──────────────────────────── */}
                      {product.approvalStatus === 'Pending' && (
                        <div className="mb-4 p-3 bg-amber-900/20 border border-amber-600/30 rounded-lg flex items-start gap-2">
                          <svg className="w-4 h-4 text-amber-400 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <p className="text-xs text-amber-300">Your product is under review. Our team will verify it shortly.</p>
                        </div>
                      )}
        ...
        {activeTab === 'orders' && (
          <div>
            {/* Verification Required Message for Unverified Vendors */}
            {!isVerified ? (
              <div className="bg-surface border border-surface-light rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-bold text-text-primary mb-2">Order Management Locked</h3>
                <p className="text-text-secondary mb-6 max-w-md mx-auto">
                  Complete your identity verification to access order management features. This helps ensure trust and security for all transactions.
                </p>
                <Link
                  to="/vendor/verification"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-black rounded-xl font-semibold transition-all duration-300 shadow-lg shadow-yellow-500/30"
                >
                  <Shield className="w-5 h-5" />
                  Start Verification
                </Link>
                        ...
                        <div key={idx} className="flex items-center justify-between py-2 border-b border-surface-light last:border-0">
                          <div className="flex items-center gap-3">
                            {item.product?.images?.[0] && (
                              <img src={resolveImageUrl(item.product.images[0])} alt={item.product.name} className="w-12 h-12 object-cover rounded-lg" />
                            )}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-bold text-text-primary">#{getDisplayOrderId(selectedOrder)}</p>
                </div>
                <div>
                ...
                <div className="space-y-3">
                  {selectedOrder.items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 p-3 bg-surface-light rounded-lg">
                      {item.product?.images?.[0] && (
                        <img src={resolveImageUrl(item.product.images[0])} alt={item.product.name} className="w-16 h-16 object-cover rounded-lg" />
                      )}
                      <div className="flex-1">
              ...
              <div className="bg-surface-light rounded-xl p-4">
                ...
                >
                  <option value="Pending Vendor Action" disabled>Pending Vendor Action</option>
