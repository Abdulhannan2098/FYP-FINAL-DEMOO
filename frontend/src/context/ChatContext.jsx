...
export const ChatProvider = ({ children }) => {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      // Only show error toast for non-404 errors (404 is expected for admins with no conversations)
  ...
  useEffect(() => {
    ...
    const handleReadReceipt = (data) => {
      const { messageId, readBy: readByUserId } = data;
      const currentUser = userRef.current;
      ...
      setMessages(prev =>
        prev.map(msg => {
          ...
          // Safety check: Only mark as 'read' if:
          // 1. The message was sent by the current user (it's their message)
          // 2. AND the reader is someone else (not the current user)
          const senderId = msg.sender?._id || msg.sender;
          const shouldMarkAsRead = senderId === currentUser?._id && readByUserId !== currentUser?._id;
